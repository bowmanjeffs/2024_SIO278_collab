...
if(gene == '16S'){
  ...
  write.csv(t(seqtab.reduced), paste0('BLE_ASVs.reduced.csv'), quote = F)
  write.csv(taxa, paste0('taxa_', gene, 'BLE_ASVs.csv'), quote = F)
  write.csv(t(seqtab.discard), paste0('BLE_ASVs_discard.csv'), quote = F)
}
...
write.csv(t(seqtab), paste0('BLE_ASVs.csv'), quote = F)
